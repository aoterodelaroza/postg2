...
subroutine sandbox_alpha(mol,mesh)
  ...
  ! aaux(3) = maxval(aeval)
  ! do i = 1, 3
  !    if (aeval(i) /= aaux(1).and.aeval(i) /= aaux(3)) then
  !       aaux(2) = aeval(i)
  !       exit
  !    endif
  ! end do
  ...
  write (iout,'("# i At        <M1^2>             <M2^2>              <M3^2>")')
  do i = 1, mol%n
     write (iout,'(I3,X,A2,X,1p,3(E18.10,X))') i, ptable(mol%z(i)), mm(1,i), mm(2,i), mm(3,i)
  enddo
  write (iout,'("#")')
