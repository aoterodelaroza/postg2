...
!> Parameters and basic routines
module param
  implicit none

  public

  public :: error ! print info, warning, or error and exit
  public :: elem2z ! atomic number from atomic symbol
  public :: z2elem ! atomic symbol from atomic number
  public :: z2nr ! number of radial points from atomic number
  public :: z2nang ! number of angular points from atomic number
  public :: z2lmax ! maximum angular momentum for sph harm fits
  public :: frevol ! atomic number to free volume
  public :: lower ! transform to lowercase
  public :: isinteger ! read an integer from a line
  public :: isreal ! read a real from a line
  public :: atoi ! convert a string to an integer
  public :: isdigit ! check if a character is a digit
  public :: atof ! convert a string to a real

  integer, parameter :: iin = 5 !< standard input LU
  integer, parameter :: iout = 6 !< standard output LU
  integer, parameter :: ihrsh = 9 !< hirshfeld charges LU
  integer, parameter :: iimo = 7 !< molecular orbitals on the grid
  integer, parameter :: iio = 8 !< other IO
  logical :: didimo 

  real*8, parameter :: pi = 4d0*datan(1d0) !< pi
  real*8, parameter :: fourpi = 4d0*pi !< 4 * pi
  real*8, parameter :: third = 1d0/3d0 !< 1/3
  real*8, parameter :: third2 = 2d0/3d0 !< 2/3
  real*8, parameter :: small = 1d-10 !< a very small number
  integer, parameter :: mline = 1024 !< maximum length of a line
  real*8, parameter :: eye3(3,3) = reshape((/1d0,0d0,0d0,0d0,1d0,0d0,0d0,0d0,1d0/),(/3,3/))
  ...
  character*2, parameter :: ptable(0:103) = (/&
     ...
     'Md','No','Lr'/) !< table of atomic symbols
  ...
  real*8, parameter ::  frepol(0:102) = (/&
     ...
     / (0.52917720859d0)**3 !< free atomic polarizabilities from CRC handbook

  ! % of HF
  real*8 :: chf  !< variable for % of HF or functional label in xdm 
  integer, parameter :: chf_blyp = -1 !< BLYP functional
  integer, parameter :: chf_b3lyp = -2 !< B3LYP functional 
  integer, parameter :: chf_bhahlyp = -3 !< BHANDHLYP functional 
  integer, parameter :: chf_camb3lyp = -4 !< CAMB3LYP functional 
  integer, parameter :: chf_pbe = -5 !< PBE functional 
  integer, parameter :: chf_pbe0 = -6 !< PBE0 functional 
  integer, parameter :: chf_lcwpbe = -7 !< LC-wPBE functional 
  integer, parameter :: chf_pw86 = -8 !< PW86PBE functional 
  integer, parameter :: chf_b971 = -9 !< B971 functional 

  ! covalent radii. 2.50 is a dummy value.
  real*8, parameter :: covrad(0:103) = (/&
     0.0d0,  0.37d0, 0.32d0,&
     1.34d0, 0.90d0, 0.82d0, 0.77d0, 0.75d0, 0.73d0, 0.71d0, 0.69d0,&
     1.54d0, 1.30d0, 1.18d0, 1.11d0, 1.06d0, 1.02d0, 0.99d0, 0.97d0,&
     1.96d0, 1.74d0, 1.44d0, 1.36d0, 1.25d0, 1.27d0, 1.39d0,&
     1.25d0, 1.26d0, 1.21d0, 1.38d0, 1.31d0,&
     1.26d0, 1.22d0, 1.19d0, 1.16d0, 1.14d0, 1.10d0,&
     2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0,&
     2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0,&
     2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0,&
     2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0,&
     2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0,&
     2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0,&
     2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0,&
     2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0,&
     2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0, 2.50d0,&
     2.50d0/) / 0.52917720859d0

contains

  !> Write a message to the standard output, and maybe die.
  !> Error routine: 0 - comment, 1 - warning, 2 - error.
...
100 format (A,"(",A,"): ",A)
  ...
  function elem2z(s) result(z)
    character*2, intent(in) :: s !< Atomic symbol
  ...
  function z2elem(z) result(s)
    integer, intent(in) :: z !< Atomic number
    character*2 :: s !< Atomic symbol

    integer :: i
    
    if (z < 0 .or. z > 103) call error("z2elem","unknown element",2)
    s = ptable(z)

  end function z2elem

  !> Atomic number to number of radial points
  function z2nr(z)
    integer, intent(in) :: z !< Atomic number
  ...
  !> Atomic number to number of angular points
  function z2nang(z)
    integer, intent(in) :: z !< Atomic number
  ...
  !> Maximum angular momentum for spherical harmonics fit
  function z2lmax(z)
    integer, intent(in) :: z !< Atomic number
    integer :: z2lmax !< Maximum angular momemtum
  ...
  !> Atomic number to free volume
  function frevol(z)

    integer, intent(in) :: z !< Atomic number
    real*8 :: frevol !< Free volume
    ...
    real*8, parameter :: frevol1(5,0:10) = reshape((/&
  ...
  function lower (string)
    ...
    character*(len(string)) :: lower !< Loweracase string
