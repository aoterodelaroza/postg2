...
!> Math tools, mesh and molecule-independent
module tools_math
...
contains
  ...
  !> Initialize math constants (factorials, etc.)
  ...
  !> The Ylm are normalized such that Y00 = 1 with respect to the
  !> angular average on the surface of the unit sphere.
  !> Storage: 
  !>             Y(0,0)  Y(1,1)  Y(2,2)
  !>             Y(1,-1) Y(1,0)  Y(2,1)   .  .  .
  !>             Y(2,-2) Y(2,-1) Y(2,0)
  !> Note: dimensions of Ylm start from zero, not one.
  !>         Y       =  YLM(K,L-M,L)     WHEN M > 0 (UPPER TRIANGLE)
  !>          LM     =  YLM(K,L,L+M)     WHEN M < 0 (LOWER TRIANGLE)
  !> Generated using recurrence relations (see numerical recipes).
  subroutine ycalc(x,nangs,ny,ylm)
    implicit none

    real*8, intent(in) :: x(3,nangs) !< positions on the unit sphere
    integer, intent(in) :: nangs !< number of points in input
    integer, intent(in) :: ny !< maximum order of the spherical harmonics
    real*8, intent(out) :: ylm(nangs,0:ny,0:ny) !< real spherical harmonics

    real*8 :: sinth(nangs),sinph(nangs),cosph(nangs)
    real*8 :: sinmph(nangs,ny),cosmph(nangs,ny)
    real*8 :: eps, c, cn
    integer :: i, j, k, l, n
    ...
    DO K=1,NANGS
       YLM(K,0,0)=1.D0
       SINTH(K)=DSQRT(X(1,K)**2+X(2,K)**2)+EPS
       SINPH(K)=X(2,K)/SINTH(K)
       COSPH(K)=X(1,K)/SINTH(K)
       SINMPH(K,1)=SINPH(K)
       COSMPH(K,1)=COSPH(K)
    end DO
    ...
    DO J=2,NY
       DO K=1,NANGS
          ...
          COSMPH(K,J)=COSPH(K)*COSMPH(K,J-1)-SINPH(K)*SINMPH(K,J-1)
       end DO
    end DO
    ...
    DO I=0,NY-1
       DO K=1,NANGS
          YLM(K,1,I+1)=(2*I+1)*X(3,K)*YLM(K,0,I)
       end DO
    end DO

    ! remaining upper triangle:
    DO I=2,NY
       DO J=I,NY
          DO K=1,NANGS
             YLM(K,I,J)=((2*J-1)*X(3,K)*YLM(K,I-1,J-1) &
                -(2*J-I-1)*YLM(K,I-2,J-2))/real(I,8)
          end DO
       end DO
    end DO

    ! normalize upper triangle with respect to theta:
    DO L=1,NY
       CN=DSQRT(2.D0*L+1.D0)
       DO K=1,NANGS
          YLM(K,L,L)=CN*YLM(K,L,L)
       end DO
    end DO
    DO J=1,NY
       DO I=0,J-1
          ...
          DO K=1,NANGS
             YLM(K,I,J)=CN*YLM(K,I,J)
          end DO
       end DO
    end DO

    ! copy upper triangle into lower triangle (ignore phase):
    DO I=1,NY
       DO J=0,I-1
          DO K=1,NANGS
             YLM(K,I,J)=YLM(K,J,I)
    ...
    ! multiply by normalized phi factors:
    DO J=1,NY
       DO I=0,J-1
          N=J-I
          DO K=1,NANGS
             YLM(K,I,J)=YLM(K,I,J)*SINMPH(K,N)
             YLM(K,J,I)=YLM(K,J,I)*COSMPH(K,N)
          end DO
       end DO
    end DO
  ...
  subroutine spline (H,Y,A,B,C,N,BCR)
    ...
    real*8, intent(in) :: h !< interval between knots
    real*8, intent(in) :: y(0:n) !< array of ordinate values
    real*8, intent(in) :: bcr !< value of function at r=infty
    integer, intent(in) :: n !< number of data points
    real*8, intent(out) :: a(0:n) !< array of coefficients of x
    real*8, intent(out) :: b(0:n) !< array of coefficients of x**2
    real*8, intent(out) :: c(0:n) !< array of coefficients of x**3
  ...
  SUBROUTINE SPLINE2(H,Y,A,B,C,N,IBC,BCL,BCR)
!
!     FITS A CUBIC SPLINE TO DATA POINTS ON THE UNIFORM R-GRID.
!     RETURNS COEFFICIENTS A,B,C DEFINING THE CUBIC POLYNOMIAL TO THE
!     RIGHT OF EACH KNOT (INCLUDING A(0),B(0),C(0)).
!
!     INPUT:
...
!         IBC  - LEFT BOUNDARY CONDITION (R=0);
!           0  - ZERO VALUE, S-WAVE CUSP
!           1  - ZERO VALUE, ZERO 1ST DERIVATIVE
!           2  - ZERO VALUE, ZERO 2ND DERIVATIVE
!         BCL  - CUSP INFORMATION IF IBC=0
!         BCR  - VALUE OF FUNCTION AT R=INFINITY (SLOPE=0)
!
!     OUTPUT:
...
!
    ...
    use tools_linpack
      ...
      DOUBLE PRECISION, INTENT(IN)  :: H,Y(0:MDATA)
      DOUBLE PRECISION, INTENT(OUT) :: A(0:MDATA),B(0:MDATA),C(0:MDATA)
      INTEGER,          INTENT(IN)  :: N,IBC
      DOUBLE PRECISION, INTENT(IN)  :: BCL,BCR
      DOUBLE PRECISION BS(-1:MDATA+1)
      DOUBLE PRECISION Y1(0:MDATA+1),T1(MDATA),T2(MDATA),T3(MDATA)
      DOUBLE PRECISION CUSP
      INTEGER I,IERR
!
      Y1(0)=Y(0)
      DO 10 I=1,N
...
!
      CALL DGTSL(N,T1,T2,T3,BS(1),IERR)
      IF(IERR.NE.0)THEN
      WRITE(IOUT,'(/)')
...
!vw
  ...
  !> Lebedev angular grid, with 110 points
  ...
  !> Lebedev angular grid, with 146 points
  ...
  !> Lebedev angular grid, with 170 points
  ...
  !> Lebedev angular grid, with 194 points
  ...
  !> Lebedev angular grid, with 302 points
